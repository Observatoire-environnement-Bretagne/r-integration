...
series_toxines <- data_toxines %>%
  ...
  mutate(rdd_code = '0000000024',
         ...
         limite_quantification = case_when(resultat == '<0,1' ~ 0.1,
                                      ...
                                      resultat == '<100' ~ 100,
                                      resultat == '<1' ~ 1),
         # Récupération du résultat numérique. Resultat = limite de quantification si < seuil
         resultat = case_when(startsWith(resultat, '<') ~ limite_quantification,
                                      resultat == TRUE ~ 1,
                                      resultat == FALSE ~ 0,
...
cyano_analyse <- series_toxines %>%
  filter(!is.na(resultat) & ! is.na(serie_code))%>%
...
cyano_analyse %>% filter(remarque_id == 10)
```
## Ecriture dans la base structure
