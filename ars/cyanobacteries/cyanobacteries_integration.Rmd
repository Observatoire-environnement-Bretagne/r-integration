...
con_mysql_local <- dbConnect(odbc::odbc(),
                             Driver=mysql_local$driver,
                             host = mysql_local$server,
                             UID = mysql_local$uid,
                             PWD = mysql_local$pwd,
...
db.insertion <- function(con,table,data) {
  ...
  DatePrel_max <- tbl(con, table) %>% summarise(DatePrel_max = max(DatePrel)) %>% pull(DatePrel_max)
...
if(params$contexte == "production") {
  ...
  db.insertion(con_mysql_local,"ars_toxines",series_toxines)
...
```{sql connection=con_postgresql_dev}

SELECT 'Fermetures', count(*), max(DatePrel), count(distinct CdPrel) FROM ars.t_interdiction_int
UNION
SELECT 'Toxines', count(*), max(DatePrel), count(distinct CdPrel) FROM ars_cyano.ars_toxines
UNION
SELECT 'Denombrements', count(*), max(DatePrel), count(distinct CdPrel) FROM oeb_ars_cyano.ars_denombrement ;

```

# Export du fichier pour la carto
```{r}
