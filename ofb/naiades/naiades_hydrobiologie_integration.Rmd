...
analyse_bio_esu <- table_indices %>%
  ...
  mutate(prelevement_code = paste(code_station_hydrobio,date_id,code_indice),
         rdd_id = 0,
         milieu_id = 3,
         fraction_id = 22,
...
insert_oeb_eau_qualite_biologique_ce <- oeb_eau_qualite_biologique_ce %>% 
  ...
  anti_join(tbl(con_eau_tbi,"oeb_eau_qualite_biologique_ce_new", by=c("Type_entitite_geographique", "Code_entitite_geographique", "Type_entitite_geographique_associee", "Code_entitite_geographique_associee", "Periode", "Serie")), copy = TRUE)%>%
...
```{r insert resultats_inconnus, eval=FALSE, include=FALSE}
dbAppendTable(conn = con_eau_tbi, name = "oeb_eau_qualite_biologique_ce_new", value = insert_oeb_eau_qualite_biologique_ce, fileEncoding="latin1")
```
