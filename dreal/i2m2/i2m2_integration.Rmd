...
db.insertion <- function(con,table,data) {
  ...
  if(dbExistsTable(con_oeb_calcul_i2m2,"oeb_i2m2")){
  ...
  data <- data %>% filter(Date > Date_max && year(Date) < as.integer(params$actualisation)+1)
...
# Package mice pour représenter les données manquantes dans le jeu de données
  mice::md.pattern(oeb_i2m2, rotate.names=T)
...
# Package mice pour représenter les données manquantes dans le jeu de données
mice::md.pattern(oeb_i2m2_outildiag, rotate.names=T)
...
db.insertion(con_oeb_calcul_i2m2,"oeb_i2m2_test",oeb_i2m2)

  dbWriteTable(con_oeb_calcul_i2m2, "oeb_i2m2_test", oeb_i2m2, overwrite=TRUE, append=FALSE,
             fileEncoding="Latin1")
