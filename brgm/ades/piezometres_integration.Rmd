...
db.insertion(con = con_mysql_local, table = "oeb_tbi_variationnappe_tmp", data = oeb_tbi_variationnappe_tmp)
...
sheets <- excel_sheets(path=paste0(params$path_dataviz,'/',fichier[1]))

import_reseau_evolution <- read_xlsx(paste0(params$path_dataviz,'/',fichier), 
          sheet=sheets[1])
```

```{r write_table_fact_resultat_site, eval=FALSE, include=FALSE}

# Migration de la table d'expertise du BRGM à partir du fichier excel

# Lecture des tables de dimensions

calendrier <- tbl(con_postgresql_dev, dbplyr::in_schema("referentiel","dim_date")) # Lecture du calendrier dans le référentiel

series <- tbl(con_postgresql_dev, dbplyr::in_schema("referentiel","dim_serie")) # Lecture des noms de séries de données

sites <- tbl(con_postgresql_dev, dbplyr::in_schema("referentiel","dim_site"))%>% filter(site_type_code == 4) %>% mutate(site_code = substr(site_code, 1,10))

# Formatage des données pour la table des résultats par site
fact_resultat_site <- import_reseau_evolution %>%
  ...
  mutate(Serie=case_when(Serie == "Niveau" ~ "nappe_niveau_mensuel_expertise",
         ...
         date_dim_id=format.Date(as.Date(dernier_jour_du_mois),"%Y%m%d"))%>%
  left_join(select(series,serie_id,serie_code), by=c("Serie" = "serie_code"), copy = TRUE)%>%
  left_join(select(sites,site_id,site_code), by="site_code", copy = TRUE)%>%
  select(site_id,periode,date_dim_id,serie_id,resultat = Resultat)

fact_resultat_site

# Ecriture en base
  #dbWriteTable(con_postgresql_dev, SQL("data_aggregees.fact_resultat_site"), fact_resultat_site, overwrite=TRUE, append=FALSE, fileEncoding="latin1")
...
fichier <- paste0("Points_reseauPz_",mois_ab[as.integer(format.Date(params$date_debut,"%m"))],format.Date(params$date_debut,"%Y"),".xls")
sheets <- excel_sheets(path=paste0(params$path_dataviz,'/',fichier[1]))

import_bulletin_piezo <- read_xls(paste0(params$path_dataviz,'/',fichier), 
          sheet=sheets[1])%>%
  ...
  mutate(date_dim_id=format.Date(as.Date(dernier_jour_du_mois),"%Y%m%d"))%>%
  # Sélection des champs du schema
  select(site_id,periode=mm_yyyy,date_dim_id,serie_id,resultat = Resultat)

import_bulletin_piezo

# Ecriture en base
  #dbWriteTable(con_postgresql_dev, SQL("data_aggregees.fact_resultat_site"), import_bulletin_piezo, overwrite=FALSE, append=TRUE, fileEncoding="latin1")
