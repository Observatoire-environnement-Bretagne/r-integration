...
fichier <- paste0("Points_reseauPz_",mois_ab[as.integer(format.Date(params$date_debut,"%m"))],format.Date(params$date_debut,"%y"),".xlsx")
sheets <- excel_sheets(path=paste0(params$path_dataviz,'/',fichier[1]))

import_bulletin_piezo <- read_xlsx(paste0(params$path_dataviz,'/',fichier), 
          sheet=sheets[1])%>%
  ...
  select(site_id,periode=mm_yyyy,date_id,serie_id,resultat = Resultat)
